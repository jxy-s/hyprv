...
namespace winrt::hyprv_app::implementation
{
    ...
    void VmTabPage::StartEnhancedFit()
    {
        if (!m_uiQueue) return;
        // Push the fit-to-window resize once immediately, then keep re-pushing
        // on a short interval until OnDesktopResized confirms the session
        // reached the Border (which calls StopEnhancedFit).
        //
        // Confirmed from captured logs (with per-call HRESULT + timestamps):
        // after OnLoginComplete, mstscax REJECTS UpdateSessionDisplaySettings
        // with E_UNEXPECTED (0x8000FFFF) until the guest's dynamic-display
        // channel is up. On a fresh first connect that's ~0.5s (one retry and
        // it's in); on a RECONNECT the guest is re-establishing the channel and
        // it takes several seconds (measured 6.7s once) — during which EVERY
        // call fails identically regardless of the requested size (a manual
        // window drag was rejected too). We can't speed up the guest-side
        // channel, so the only lever is to catch the instant it opens: poll on
        // a TIGHT interval (200ms) so the first post-readiness call lands within
        // ~200ms instead of lagging up to a full slow interval. Retries are
        // cheap — one IPC frame + an instant E_UNEXPECTED return — and the
        // popup stays put at the login size meanwhile (no churn), so a tight
        // poll is pure upside. Each tick still carries a 1px height nudge as
        // insurance: once mstscax is ready it acts on a size DELTA, and the
        // OnDesktopResized echo then converges to full size via the normal path
        // and calls StopEnhancedFit.
        m_enhancedFitTicks = 0;
        if (m_client) m_client->InvalidateSessionDisplayCache();
        UpdateRdphostBounds();
        if (!m_enhancedFitTimer)
        {
            m_enhancedFitTimer = m_uiQueue.CreateTimer();
            m_enhancedFitTimer.IsRepeating(true);
            m_enhancedFitTimer.Interval(std::chrono::milliseconds(200));
            auto weak = get_weak();
            m_enhancedFitTimer.Tick([weak](auto const&, auto const&) {
                auto self = weak.get();
                if (!self) return;
                // Keep retrying for ~12s (60 ticks × 200ms) to outlast the
                // reconnect dynamic-channel lag. Stop quietly on timeout — a
                // future window resize still re-fits via the normal path.
                if (++self->m_enhancedFitTicks > 60 ||
                    !self->m_enhancedReady || !self->m_client)
                {
                    self->StopEnhancedFit();
                    return;
                }
                int sx = 0, sy = 0, bw = 0, bh = 0;
                if (!self->ComputeBorderScreenRect(sx, sy, bw, bh)) return;
                UINT dpi = self->m_dpiOverridePercent
                    ? static_cast<UINT>(self->m_dpiOverridePercent * 96 / 100)
                    : GetDpiForWindow(self->m_windowHwnd);
                // Alternate a 1px shrink so consecutive requests always differ
                // — the delta is what mstscax acts on once ready; the 1px is
                // reclaimed when OnDesktopResized re-runs UpdateRdphostBounds at
                // full bh.
                const int nudge = (self->m_enhancedFitTicks & 1) ? 1 : 0;
                self->m_client->InvalidateSessionDisplayCache();
                self->m_client->UpdateSessionDisplaySettings(
                    static_cast<uint32_t>(bw),
                    static_cast<uint32_t>(bh > nudge ? bh - nudge : bh),
                    dpi, dpi);
            });
        }
        m_enhancedFitTimer.Stop();
        m_enhancedFitTimer.Start();
    }

    void VmTabPage::StopEnhancedFit()
    {
        if (m_enhancedFitTimer) m_enhancedFitTimer.Stop();
        m_enhancedFitTicks = 0;
    }
    ...
    void VmTabPage::SetPlaceholderVisible(bool visible)
    {
        using namespace Microsoft::UI::Xaml;
        // infoRoot itself is ALWAYS present — the VM name draws in every state
        // (and feeds the DWM taskbar thumbnail while a session is live, since
        // the popup is a separate window DWM doesn't capture). "visible" here
        // means "show the status group (spinner/state/hint/buttons)"; when a
        // live session covers the surface we hide that group and make the
        // surface non-hit-testable so its transparent area can't eat clicks in
        // the popup's letterbox margins. The name row stays untouched.
        if (auto g = placeholderStatusGroup())
            g.Visibility(visible ? Visibility::Visible : Visibility::Collapsed);
        if (auto ir = infoRoot())
            ir.IsHitTestVisible(visible);
    }
    ...
    void VmTabPage::RenderErrorPlaceholder(std::wstring const& vmName)
    {
        using namespace Microsoft::UI::Xaml;
        if (!infoRoot()) return;
        if (auto sp = placeholderSpinner()) sp.Visibility(Visibility::Collapsed);
    ...
    {
        using namespace Microsoft::UI::Xaml;
        if (!infoRoot()) return;   // XAML not loaded yet
                                      ...
                                      bool showSpinner = false)
        {
            ...
            if (auto sp = placeholderSpinner())
                sp.Visibility(showSpinner ? Visibility::Visible
        ...
        // "deleted VM" from "VM exists, we just don't know it yet." Show the
        // single placeholder panel with a spinner ("Loading…") until the first
        // snapshot lands; UpdatePlaceholderAndClient re-runs on every
        // OnVmManagerChanged so we'll naturally fall through to the right
        // branch once the cache is populated. Skip the rdphost spawn here.
        // (UpdateVmTabHeader has already set the name; if it isn't known yet it
        // shows "Loading...", so the headline + spinner read consistently.)
        if (!vmOpt && !vmm.HasFirstSnapshot())
        {
            TearDownClient();
            // Headline = whatever name we have. UpdateVmTabHeader sets
            // placeholderVmName (resolving to "Loading..." before the first
            // poll), so reuse it as the headline; spinner underneath conveys
            // "still working".
            std::wstring coldName = placeholderVmName()
                ? std::wstring{ placeholderVmName().Text() } : std::wstring{};
            if (coldName.empty()) coldName = L"Loading...";
            showPlaceholder(coldName, std::wstring{}, std::wstring{},
                            /*showStartButton*/ false, /*showSpinner*/ true);
            m_lastState = hyprv::app::vm::VmState::Unknown;
            return;
        }
    ...
    void VmTabPage::StartConnection()
    {
        ...
        m_client->OnConnected = [queue, weakSelf](hyprv::app::ConnectedInfo const& c)
        {
            ...
            auto text = winrt::hstring{ std::format(
                ...
                c.enhanced ? 1 : 0, c.desktopWidth, c.desktopHeight) };
            const bool enhanced = c.enhanced;
            bool ok = queue.TryEnqueue([weakSelf, text, enhanced] {
                ...
                if (auto self = weakSelf.get())
                {
                    ...
                    self->m_everConnected = true;
                    // An enhanced session that reached Connected (even just the
                    // enhanced LOGIN screen, before login-complete) counts as
                    // "enhanced works here" — so a later break/drop won't arm
                    // the enhanced→basic cooldown. Without this, breaking into
                    // the kernel debugger AT the enhanced login screen (Connected
                    // but not yet OnEnhancedReady) fell back to basic. See
                    // m_everEnhanced + the cooldown gate in OnDisconnected.
                    if (enhanced) self->m_everEnhanced = true;
            ...
            queue.TryEnqueue([weakSelf, w, h] {
                if (auto self = weakSelf.get())
                {
                    ...
                    if (w > 0 && h > 0)
                    {
                        ...
                        self->UpdateRdphostBounds();
                        // mstscax confirmed a new session size. If we're mid
                        // enhanced fit-to-window retry and the session now
                        // (roughly) fills the Border, stop re-pushing — the
                        // guest snaps to the nearest valid resolution so an
                        // exact match isn't guaranteed; a small tolerance
                        // avoids both an early stop and an endless re-push.
                        if (self->m_enhancedFitTicks || self->m_enhancedFitTimer)
                        {
                            int bx = 0, by = 0, bw = 0, bh = 0;
                            if (self->ComputeBorderScreenRect(bx, by, bw, bh))
                            {
                                const int tol = 8;
                                if (self->m_sessionW >= bw - tol &&
                                    self->m_sessionH >= bh - tol)
                                    self->StopEnhancedFit();
                            }
                        }
                    }
            ...
            queue.TryEnqueue([weakSelf, text, r] {
                if (auto self = weakSelf.get())
                {
                    ...
                    if (r)
                    {
                        ...
                        // Remember this tab has had a working enhanced session,
                        // so a later enhanced drop (reboot / kernel-debugger
                        // break) won't arm the enhanced→basic cooldown and fall
                        // us to a basic session — we reconnect straight back to
                        // enhanced. See m_everEnhanced + the cooldown gate in
                        // OnDisconnected.
                        self->m_everEnhanced = true;
                        // mstscax often silently rejects the first
                        // UpdateSessionDisplaySettings right after
                        // OnLoginComplete (still finishing the enhanced
                        // transition) and the IPC cache then pins the rejected
                        // dims, so identical re-sends short-circuit and the
                        // session stays small until the user wiggles the
                        // window. StartEnhancedFit re-pushes the full-Border
                        // resize on a short repeating timer (dropping the cache
                        // each tick) until OnDesktopResized confirms the
                        // session reached the Border — no manual resize needed.
                        self->StartEnhancedFit();
                    }
                }
            });
        };
        m_client->OnDisconnected = [queue, weakSelf](hyprv::app::DisconnectedInfo const& d)
        {
            ...
            queue.TryEnqueue([weakSelf, text, fatal, desc] {
                if (auto self = weakSelf.get())
                {
                    ...
                    // ALSO skip the cooldown entirely once this tab has EVER had
                    // a working enhanced session (m_everEnhanced): a later
                    // enhanced drop is a transient (reboot, or a KERNEL-DEBUGGER
                    // break that freezes the guest), NOT "enhanced is unusable
                    // here", so falling to basic for 20s is wrong — keep trying
                    // enhanced. The cooldown still applies to a never-enhanced
                    // tab (OS install / first boot), where basic catches the
                    // boot screen.
                    ...
                    if (running)
                    {
                        ...
                        else if (self->m_everConnected)
                        {
                            // This tab HAS connected before, just not on this
                            // attempt. A run of fatal failures on a still-
                            // Running VM that we've reached before is a
                            // transient stall — overwhelmingly the user broke
                            // into a KERNEL DEBUGGER (guest CPU halted, console
                            // refuses/drops). Do NOT escalate to the give-up
                            // error view; keep retrying on the debounce so the
                            // session auto-revives the instant the debugger
                            // resumes. (A never-connected tab still escalates
                            // below — that's a genuinely bad/unsupported console.)
                            self->m_connectFailures = 0;
                        }
                        else if (fatal &&
        ...
        // Enhanced initial framebuffer.
        //
        // The post-login UpdateSessionDisplaySettings resize is unreliable on a
        // RECONNECT: mstscax returns E_UNEXPECTED until the guest rebuilds its
        // dynamic-display channel, which can take ~7s (confirmed via logged
        // HRESULTs + timestamps) — so a session that connects small and relies
        // on that resize sits small for several seconds. The connect-time
        // DesktopWidth/Height, by contrast, is part of the initial RDP
        // negotiation and is ALWAYS honored immediately. So when the Border is
        // already measured (the common close-tab/reopen case), connect at the
        // FULL Border size — the session is full-size from the first frame and
        // the post-login resize becomes a no-op safety net (StartEnhancedFit
        // still runs to handle the rare window-resized-during-login case).
        //
        // Fallback (Border NOT yet measured — cold-start race, gotcha #14):
        // keep the old scaled-1024×768 placeholder so the guest's logon UI
        // still centers in a sane logical desktop (gotcha #49); the retry +
        // OnRdpHostSizeChanged expand it once the Border resolves.
        if (wantEnhanced)
        {
            if (borderValid)
            {
                opts.desktopWidth  = static_cast<uint16_t>(bw);
                opts.desktopHeight = static_cast<uint16_t>(bh);
                m_sessionW = bw;
                m_sessionH = bh;
            }
            else
    ...
    void VmTabPage::UpdateVmTabHeader()
    {
        ...
        // Keep the always-present info-surface name in sync with the live name
        // (it's the single VM-name element now; drawn behind the rdpHost popup
        // and captured by the DWM taskbar thumbnail). Guarded separately from
        // m_tabItem so it updates even before the page is hosted in a
        // TabViewItem.
        if (auto nm = placeholderVmName())
            nm.Text(header);
        if (!m_tabItem) return;
