...
namespace winrt::hyprv_app::implementation
{
    ...
    void WelcomePage::RenderAllVms()
    {
        ...
        {
            hint.Visibility(rendered == 0 && !filter.empty()
                ? Microsoft::UI::Xaml::Visibility::Visible
                : Microsoft::UI::Xaml::Visibility::Collapsed);
        }

        // Loading indicator: shown only while we've never seen any VMs.
        // Once the first non-empty snapshot lands, hide forever — a later
        // empty list is real (no VMs left), not "still loading".
        // SCOPED to the ALL VMs section: only the table waits on the poll
        // (the inline `allVmsLoading` spinner replaces it during cold-start),
        // so the rest of the page — Recents + Remote Hosts, which come from
        // Settings, not the VM poll — stays visible and usable immediately.
        if (!m_firstSnapshotRendered && !vms.empty())
            m_firstSnapshotRendered = true;
        const auto loadingVis = m_firstSnapshotRendered
            ? Microsoft::UI::Xaml::Visibility::Collapsed
            : Microsoft::UI::Xaml::Visibility::Visible;
        const auto contentVis = m_firstSnapshotRendered
            ? Microsoft::UI::Xaml::Visibility::Visible
            : Microsoft::UI::Xaml::Visibility::Collapsed;
        if (auto loading = allVmsLoading()) loading.Visibility(loadingVis);
        if (auto table   = allVmsTable())   table.Visibility(contentVis);
    }
