...
void HyprvAppLog(const wchar_t* fmt, ...)
{
    ...
    // Prefix every line with a wall-clock HH:MM:SS.mmm timestamp so timing
    // gaps (e.g. how long mstscax takes to honor a resize after login) are
    // measurable from the log. Child rdphost lines are forwarded verbatim and
    // get the timestamp here, at write time, like every other line.
    SYSTEMTIME st; GetLocalTime(&st);
    fwprintf(g_log, L"[%02u:%02u:%02u.%03u] ",
...
namespace winrt::hyprv_app::implementation
{
    ...
    void MainWindow::OnActivated(Microsoft::UI::Xaml::WindowActivatedEventArgs const&)
    {
        ...
        this->Closed([changedTok, errorTok, weakSelf](
            ...
            if (auto self = weakSelf.get())
            {
                // Close the backdrop controllers NOW, while the XAML target +
                // DispatcherQueue are still alive. If left to the framework's
                // own DispatcherQueueShutdownStarting teardown it AVs deep in
                // Microsoft.UI.Xaml (crash-dump confirmed) — see TearDownBackdrop.
                self->TearDownBackdrop();
    ...
    {
        // Detach the backdrop target + Close() each live controller. MUST run
        // before the DispatcherQueue shuts down (we call it from the window
        // Closed handler) — otherwise the framework's own
        // DispatcherQueueShutdownStarting handler closes the controller against
        // a half-torn-down XAML target and AVs deep in Microsoft.UI.Xaml
        // (SystemBackdropInternal::BaseController::CloseInternal ->
        // RemoveAllTargetsInternal -> null deref; confirmed via crash dump).
        // Idempotent: nulls each controller so a second call is a no-op.
        auto target = this->try_as<
            winrt::Microsoft::UI::Composition::ICompositionSupportsSystemBackdrop>();
        if (m_acrylicController)
        {
            if (target) m_acrylicController.RemoveSystemBackdropTarget(target);
            m_acrylicController.Close();
            m_acrylicController = nullptr;
        }
        if (m_micaController)
        {
            if (target) m_micaController.RemoveSystemBackdropTarget(target);
