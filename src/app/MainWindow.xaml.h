...
namespace winrt::hyprv_app::implementation
{
    ...
    public:
        ...
        // Close + null the live backdrop controllers (Mica/Acrylic) and detach
        // their backdrop target. Called from ApplyAppearance (before a rebuild)
        // AND from the window Closed handler. If we DON'T close them before the
        // DispatcherQueue shuts down, the framework auto-closes them during
        // DispatcherQueueShutdownStarting against a half-torn-down XAML target
        // and AVs in Microsoft.UI.Xaml (confirmed via crash dump — see gotcha
        // about backdrop shutdown teardown).
