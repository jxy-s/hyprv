...
namespace hyprv::app::vm
{
    ...
    bool VMManager::TryConnect()
    {
        try
        {
            ...
            // Do NOT gate on Msvm_VirtualSystemManagementService being present here.
            // GetInstance returns empty for that singleton even on a host with
            // Hyper-V perfectly enabled but no VMs (verified false-positive), so
            // treating its absence as "Hyper-V not enabled" wrongly showed the error
            // panel on healthy hosts. We don't try to infer host health: the poll
            // simply runs, and a host with no VMs shows the empty state. The
            // genuinely-uninstalled case (no root\virtualization\v2 namespace) is
            // still caught by the ConnectServer throw above (-> HyperVUnavailable).
    ...
    void VMManager::PollLoop()
    {
        ...
        // Initial refresh — kicks the first OnChanged before the 1 s wait.
        {
            ...
            try { UpdateAll(vsms, perfScope.get()); } catch (...) {}
            NotifyChanged();   // ALWAYS notify, even if UpdateAll threw — the UI must
                               // leave "Loading VMs…" (showing whatever m_vms holds,
                               // empty included) rather than spin forever.
        ...
        while (!m_stop.load())
        {
            ...
            uint64_t servicing = m_pollReq.load();
            try   { UpdateAll(vsms, perfScope.get()); }
            catch (...) { /* never escape the poll thread */ }
            NotifyChanged();   // always notify (see initial refresh) — keeps the UI
                               // live + out of the loading state even on a bad poll.
