...
namespace
{
    // Resolve %LOCALAPPDATA%\hyprv (creating it on first run). On MSIX this
    // path is transparently redirected by the OS into the package's
    // LocalState folder — code stays identical across packaging modes.
    std::filesystem::path LocalAppDataDir()
    {
        PWSTR p = nullptr;
        HRESULT hr = SHGetKnownFolderPath(FOLDERID_LocalAppData,
                                           KF_FLAG_CREATE, nullptr, &p);
        if (FAILED(hr) || !p)
        {
