...
namespace hyprv::rdphost
{
                            ...
                            const std::string& username)
    {
        ...
        // Disable mstscax's built-in auto-reconnect here too — a real RDP host
        // that goes unreachable should surface as a prompt OnDisconnected so
        // the parent can show its placeholder / error path, not sit on the
        // Microsoft "attempting to reconnect" overlay. (Same rationale as the
        // VM-console path in OnConnectLocalVm.)
        adv6->put_EnableAutoReconnect(VARIANT_FALSE);
                                   ...
                                   const std::string&, const std::string&)
    {
        ...
        adv6->put_SmartSizing(VARIANT_FALSE);
        // Disable mstscax's BUILT-IN auto-reconnect. By default it's ON, so a
        // transport stall (notably: the user breaks into a KERNEL DEBUGGER and
        // the guest CPU halts — VM stays "Running" but no RDP frames flow)
        // makes mstscax paint its own "The connection has been lost. Attempting
        // to reconnect your session…" overlay and silently run an internal
        // retry loop, WITHOUT firing OnDisconnected. Consequences: (1) hyprv
        // can't drop to its own clean "Connecting…" placeholder because it
        // never learns the session stalled, and (2) when the debugger resumes,
        // mstscax's internal reconnect state machine competes with / slows
        // hyprv's own reconnect. Turning it OFF makes a stall surface promptly
        // as OnDisconnected → hyprv hides the dead surface, shows its
        // placeholder, and drives its own debounced reconnect on a fresh
        // control with no stale state to unwind. Lives on
        // IMsRdpClientAdvancedSettings2 (inherited by adv6).
        adv6->put_EnableAutoReconnect(VARIANT_FALSE);
    ...
    void RdpHost::OnUpdateSessionDisplaySettings(const hyprv::ipc::UpdateSessionDisplaySettings& cmd)
    {
        ...
        // Log EVERY call (not just failures) via the IPC-forwarded logger so
        // the parent's hyprv.log shows whether mstscax accepted the resize.
        // mstscax returns success but silently ignores the call until the
        // dynamic-resize virtual channel to the guest is up (notably on a
        // reconnect, where it lags several seconds after OnLoginComplete) —
        // this hr is the only signal that distinguishes "rejected" from
        // "accepted-but-no-echo-yet". (OutputDebugStringW alone never reaches
        // our log.)
        HyprvLog(L"[host] UpdateSessionDisplaySettings(%u,%u scale=%u) hr=0x%08lx",
            cmd.width, cmd.height, cmd.desktopScale, hr);
    }
